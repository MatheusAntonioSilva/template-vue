                          ...
                          <!-- Form -->
                            ...
                            <h3 class="dark-grey-text text-center">
                             ...
                             <div v-if="messagemSucess==true" class="alert alert-success" role="alert">
                                <p class="messagem">Empresa alterada com sucesso! Bem vindo ao EuAgendei</p>
                              </div>
                            ...
                            <div class="text-center">
                              <button v-on:click="updateCompany" class="btn btn-indigo">Confirmar</button>
                            </div>
                          <!-- Form -->
...
export default {
  ...
  mounted () {
    var id = this.$route.params.companyId
    this.$http.get('http://localhost:8000/api/company/'+id).then(res => {
      console.log(res.body.company)
      this.razsocial = res.body.company.razsocial,
      this.fantasia = res.body.company.fantasia,
      this.cnpj = res.body.company.cnpj,
      this.rua = res.body.company.endereco,
      this.numero = res.body.company.numero,
      this.cidade = res.body.company.cidade
      this.estado = res.body.company.uf,
      this.cep = res.body.company.cep,
      this.contato = res.body.company.contato,
      this.email = res.body.company.email
  ...
  methods: {
    updateCompany: function (event) {
      ...
      if (this.validaRegistro() !== false) {
        this.$http.put('http://localhost:8000/api/company/'+id, {
          razsocial: this.razsocial,
          fantasia: this.fantasia,
          cnpj: this.cnpj,
          cep: this.email,
